...
function canonicalTool(nativeName) {
...
function collectEvent(provider, value, calls, ordinal) {
  ...
  if (provider === "claude" && Array.isArray(value.message?.content)) for (const part of value.message.content) {
    if (part?.type === "tool_use") addCall(calls, part.name, part.id, ordinal);
  }
}
...
export function observeToolUsage(provider, stdout, providerConfig = {}) {
  ...
  if (!observable) return Object.freeze({ status: "unavailable", enforcement: "unknown", source: `${provider}:tool-events-not-exposed`, native_tools: [], canonical_tools: [], unknown_native_tools: [] });
  const calls = new Map();
  let parseErrors = 0, ordinal = 0, parsedEvents = 0;
  for (const line of String(stdout ?? "").split(/\r?\n/).filter(line => line.trim())) {
    ordinal += 1;
    try { collectEvent(provider, JSON.parse(line), calls, ordinal); parsedEvents += 1; }
    catch { parseErrors += 1; }
  }
