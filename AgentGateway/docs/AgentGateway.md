<document id="agent_gateway" status="accepted" authority="zodchi" version="0.6.12" language="en">
  ...
  <section id="browser_mcp_profile" status="accepted">
    ...
    <verification>Run `node tools/mcp-browser-smoke.mjs --project &lt;root&gt; --policy &lt;local-policy&gt; --provider &lt;codex|claude|opencode&gt; --profile &lt;profile&gt; --keep --report &lt;evidence.json&gt;`. The smoke admits only context input, verifies that the named MCP server was actually carried, observes the browser sentinel independently, and accepts capture evidence only from a retained PNG whose IHDR is at least the requested 800×600 viewport before recording its digest.</verification>
