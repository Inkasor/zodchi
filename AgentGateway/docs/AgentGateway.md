<document id="agent_gateway" status="accepted" authority="zodchi" version="0.6.12" language="en">
  ...
  <responsibilities status="accepted">
    ...
    <rule id="executor_capability_contract">Every invocation carries an explicit capability requirement. Gateway derives the assigned profile's provider-specific capability matrix and starts the provider only when every required capability is technically available and every forbidden capability is technically unavailable. A local acceptedDeclarativeBoundaries exception may admit one named role's forbidden declarative boundary with an owner-readable reason; it never satisfies a required capability, never applies to another role, and remains explicit in readiness and the receipt.</rule>
  ...
  <security status="accepted">
    ...
    <rule id="claude_environment_scope">Claude's strict temporary MCP file is reported as mcp-config-only. Gateway does not replace Claude's HOME and therefore does not claim to isolate its ambient settings, hooks or skills.</rule>
  </security>
