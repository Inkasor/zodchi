...
export class Runtime {
  ...
  create(message, context = {}) {
    ...
    try {
      ...
      // The binding says which installation answered this message and what it inherited to do it. Without
      // it the record cannot distinguish a run this project asked for from one another project's shell
      // routed here.
      appendEvent(this.db, { entityType: "workflow_run", entityId: runId, kind: "created", payload: { source, event_key: eventKey, ...(context.event_fields?.length ? { event_fields: context.event_fields } : {}), ...(context.binding ? { binding: context.binding } : {}) } });
