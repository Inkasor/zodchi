...
export async function executeStructuredWork({ runtime, runId, classification, definition, discovery, message, responseLanguage = "en", taskRoot, gatewayCall, gateRunner = runProjectGate, approvalGranted = false }) {
  ...
  plannerPackage.collection_contract = {
    ...
    decomposition_rule: "A broad static trace that needs more than eight source paths or separately covers producers, consumers and tests must be split into sequential read-only investigation steps of at most eight paths each. Reuse the registered read-only role when appropriate. Every later step receives compact structured findings from earlier steps, so a final synthesis step can combine the chain without putting the whole repository into one invocation.",
