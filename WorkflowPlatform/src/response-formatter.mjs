...
export function workflowMessage(key, language = "en") {
  ...
  const values = {
    ...
    approvalStale: ru ? "План или доказательный checkpoint изменился после запроса подтверждения. Старое подтверждение не применено; открыто новое решение для текущего точного состояния." : "The plan or evidence checkpoint changed after approval was requested. The old approval was not applied; a new decision is open for the current exact state.",
