...
export function workflowMessage(key, language = "en") {
  ...
  const values = {
    ...
    approvalAfterWork: ru ? "Решение записано. Прогон уже выполнил свои шаги и остановился на решении после работы; автоматическое продолжение с этого места пока не поддерживается." : "The decision is recorded. This run had already completed its steps and stopped at a decision that follows the work; continuing it automatically is not supported yet.",
