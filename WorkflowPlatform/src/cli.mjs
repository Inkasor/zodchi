...
import { projectRuntimeReadiness } from "./runtime-readiness.mjs";
...
else if (process.argv[2] === "project-readiness") { const runtime = new Runtime(args.db ?? settings.databasePath); try { const result = projectRuntimeReadiness(runtime.db, args.project); console.log(JSON.stringify(result, null, 2)); process.exitCode = result.status === "ready" ? 0 : 1; } finally { runtime.db.close(); } }
...
else if (process.argv[2] === "goal-list") { const runtime = new Runtime(args.db ?? settings.databasePath); try { console.log(JSON.stringify(listStrategicState(runtime.db, args.project), null, 2)); } finally { runtime.db.close(); } }
else if (process.argv[2] === "goal-create") { const runtime = new Runtime(args.db ?? settings.databasePath); try { console.log(JSON.stringify(createGoal(runtime.db, { projectId: args.project, title: args.title, confirmedBy: args["confirmed-by"] }), null, 2)); } finally { runtime.db.close(); } }
else if (process.argv[2] === "goal-status") { const runtime = new Runtime(args.db ?? settings.databasePath); try { console.log(JSON.stringify(setGoalStatus(runtime.db, { goalId: args.goal, status: args.status, confirmedBy: args["confirmed-by"] }), null, 2)); } finally { runtime.db.close(); } }
else if (process.argv[2] === "stage-create") { const runtime = new Runtime(args.db ?? settings.databasePath); try { console.log(JSON.stringify(createStage(runtime.db, { projectId: args.project, goalId: args.goal, stageKey: args.key, title: args.title, ordinal: Number(args.ordinal ?? 0), confirmedBy: args["confirmed-by"] }), null, 2)); } finally { runtime.db.close(); } }
else if (process.argv[2] === "stage-status") { const runtime = new Runtime(args.db ?? settings.databasePath); try { console.log(JSON.stringify(setStageStatus(runtime.db, { stageId: args.stage, status: args.status, confirmedBy: args["confirmed-by"] }), null, 2)); } finally { runtime.db.close(); } }
else if (process.argv[2] === "task-state") { const runtime = new Runtime(args.db ?? settings.databasePath); try { console.log(JSON.stringify(taskStateProjection(runtime.db, args.run), null, 2)); } finally { runtime.db.close(); } }
else if (process.argv[2] === "interaction-read") { const runtime = new Runtime(args.db ?? settings.databasePath); try { console.log(JSON.stringify(addressedInteraction(runtime.db, args.run, args.interaction), null, 2)); } finally { runtime.db.close(); } }
else if (process.argv[2] === "event-read") { const runtime = new Runtime(args.db ?? settings.databasePath); try { console.log(JSON.stringify(addressedEvent(runtime.db, args.run, args.event), null, 2)); } finally { runtime.db.close(); } }
else if (process.argv[2] === "external-tool-list") { const runtime = new Runtime(args.db ?? settings.databasePath); try { console.log(JSON.stringify(externalTools(runtime.db, args.project), null, 2)); } finally { runtime.db.close(); } }
else if (process.argv[2] === "external-tool-register") { const runtime = new Runtime(args.db ?? settings.databasePath); try { console.log(JSON.stringify(registerExternalTool(runtime.db, { projectId: args.project, name: args.name, transport: args.transport, endpoint: args.endpoint, pinnedVersion: args.version, readOnlyMode: args["read-only-config"] ? JSON.parse(fs.readFileSync(args["read-only-config"], "utf8")) : null, arbitraryExecution: args["arbitrary-execution"] === true, containsModel: args["contains-model"] === true, selfLiftableBoundary: args["self-liftable-boundary"] === true, doublesAsProvider: args["doubles-as-provider"] === true }), null, 2)); } finally { runtime.db.close(); } }
