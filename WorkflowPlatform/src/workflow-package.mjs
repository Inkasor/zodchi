...
export function validateWorkflowPackage(value) {
  ...
  if (value.schema_version !== 1) throw new Error("workflow_package: unsupported schema_version");
  ...
  exactObject(value.catalogs, ["work_types", "artifact_types", "quality_modes", "planning_levels"], "catalogs");
  const catalogs = {};
  ...
  for (const role of value.roles) { exactObject(role, ["key", "name", "contract"], `role.${role.key}`); exactObject(role.contract, contractFields, `role.${role.key}.contract`); const c = role.contract; if (!SEMVER.test(c.version) || !SEMVER.test(c.prompt_template_version) || !Number.isInteger(c.context_limit_bytes) || c.context_limit_bytes < 1024 || !Number.isInteger(c.max_calls) || c.max_calls < 1 || !Number.isInteger(c.max_correction_cycles) || c.max_correction_cycles < 0 || !Number.isInteger(c.timeout_seconds) || c.timeout_seconds < 1) throw new Error(`role.${role.key}: invalid contract`); for (const workType of array(c.allowed_work_types, "contract.allowed_work_types")) if (!catalogs.work_types.has(workType)) throw new Error(`role.${role.key}: missing work type ${workType}`); for (const artifact of array(c.allowed_artifact_types, "contract.allowed_artifact_types")) if (!catalogs.artifact_types.has(artifact)) throw new Error(`role.${role.key}: missing artifact ${artifact}`); }
  ...
  for (const check of value.checks) { exactObject(check, ["key", "name", "runner", "kind", "config", "timeout_seconds", "bindings"], `check.${check.key}`); if (!Number.isInteger(check.timeout_seconds) || check.timeout_seconds < 1) throw new Error(`check.${check.key}: invalid timeout`); for (const binding of array(check.bindings, "check.bindings")) { exactObject(binding, ["quality_mode_key", "artifact_type_key", "required"], "check.binding"); if (!catalogs.quality_modes.has(binding.quality_mode_key) || (binding.artifact_type_key && !catalogs.artifact_types.has(binding.artifact_type_key))) throw new Error(`check.${check.key}: missing binding dependency`); } }
...
export function serializeWorkflowPackage(value) { const valid = validateWorkflowPackage(value); return `<workflow_package key="${escapeXml(valid.key)}" version="${escapeXml(valid.version)}" schema_version="1">\n  <purpose>${escapeXml(valid.purpose)}</purpose>\n  <payload format="application/json">${escapeXml(stableJson(valid))}</payload>\n</workflow_package>\n`; }
export function parseWorkflowPackage(source) { const root = parseLimitedXml(source); if (root.name !== "workflow_package") throw new Error("PACKAGE_ROOT_INVALID"); const attributes = exactAttributes(root, ["key", "version", "schema_version"]), [purpose, payload] = exactChildren(root, ["purpose", "payload"]); exactAttributes(purpose, []); exactAttributes(payload, ["format"]); if (purpose.children.length || payload.children.length || payload.attributes.format !== "application/json") throw new Error("PACKAGE_ENVELOPE_INVALID"); const value = validateWorkflowPackage(JSON.parse(payload.text)); if (attributes.schema_version !== "1" || attributes.key !== value.key || attributes.version !== value.version || purpose.text !== value.purpose) throw new Error("PACKAGE_ENVELOPE_MISMATCH"); return value; }
...
function packageDiff(db, projectId, value) {
  ...
  for (const document of value.documents) { const collision = db.prepare("SELECT id FROM project_documents WHERE project_id=? AND path=?").get(projectId, document.path); if (collision && mappedKey(db, projectId, "document", collision.id) !== document.key) throw new Error(`PACKAGE_COLLISION: document_path:${document.path}`); add("document", document.key, collision); }
...
function applyBindingsAndPolicies(db, proposal, projectId, value, workflowMap) {
  ...
  for (const document of value.documents) {
    ...
    db.prepare("INSERT INTO project_documents(id,project_id,path,document_type,authority,status,active) VALUES(?,?,?,?,?,?,1) ON CONFLICT(id) DO UPDATE SET path=excluded.path,document_type=excluded.document_type,authority=excluded.authority,status=excluded.status,active=1").run(documentId, projectId, document.path, document.type, document.authority, document.status);
