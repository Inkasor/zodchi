...
function buildPackage(db, projectId, workflowFilter) {
  ...
  const workflows = workflowRows.map(row => ({
    key: workflowKeys.get(row.id), name: row.name, default_quality: row.default_quality, default_level: row.default_level, status: row.status, discovery: parseJson(row.discovery_json, { git: false }),
...
export function validateWorkflowPackage(value) {
  if (value?.schema_version !== PACKAGE_SCHEMA_VERSION) {
    if ([1, 2, 3].includes(value?.schema_version)) throw new Error(`WORKFLOW_PACKAGE_SCHEMA_MIGRATION_REQUIRED: schema_version ${value.schema_version} -> 4`);
  ...
  const contractFields = ["version", "purpose", "boundaries", "allowed_work_types", "allowed_artifact_types", "allowed_tools", "allowed_skills", "allowed_mcp_servers", "native_instruction_files", "required_checks", "allowed_transitions", "allowed_profile_keys", "context_limit_bytes", "max_calls", "max_correction_cycles", "timeout_seconds", "result_schema_key", "prompt_template_version", "escalation"];
  for (const role of value.roles) {
    exactObject(role, ["key", "name", "contract"], `role.${role.key}`); exactObject(role.contract, contractFields, `role.${role.key}.contract`); const c = role.contract;
    if (!String(c.purpose ?? "").trim()) throw new Error(`role.${role.key}: executable purpose is required`);
    if (!SEMVER.test(c.version) || !SEMVER.test(c.prompt_template_version) || !Number.isInteger(c.context_limit_bytes) || c.context_limit_bytes < 1024 || !Number.isInteger(c.max_calls) || c.max_calls < 1 || !Number.isInteger(c.max_correction_cycles) || c.max_correction_cycles < 0 || !Number.isInteger(c.timeout_seconds) || c.timeout_seconds < 1) throw new Error(`role.${role.key}: invalid contract`);
    for (const workType of array(c.allowed_work_types, "contract.allowed_work_types")) if (!catalogs.work_types.has(workType)) throw new Error(`role.${role.key}: missing work type ${workType}`);
    for (const artifact of array(c.allowed_artifact_types, "contract.allowed_artifact_types")) if (!catalogs.artifact_types.has(artifact)) throw new Error(`role.${role.key}: missing artifact ${artifact}`);
    const knownTools = new Set(["apply_patch", "exec_command"]);
    for (const tool of array(c.allowed_tools, "contract.allowed_tools")) if (!knownTools.has(tool)) throw new Error(`role.${role.key}: unknown allowed tool ${tool}`);
    for (const field of ["allowed_skills", "allowed_mcp_servers", "native_instruction_files"]) for (const item of array(c[field], `contract.${field}`)) if (typeof item !== "string" || !item.trim()) throw new Error(`role.${role.key}: invalid ${field}`);
    for (const file of c.native_instruction_files) if (path.isAbsolute(file) || file.split(/[\\/]/u).includes("..")) throw new Error(`role.${role.key}: unsafe native instruction path ${file}`);
  }
...
export function parseWorkflowPackage(source) { const root = parseLimitedXml(source); if (root.name !== "workflow_package") throw new Error("PACKAGE_ROOT_INVALID"); const attributes = exactAttributes(root, ["key", "version", "schema_version"]), [purpose, payload] = exactChildren(root, ["purpose", "payload"]); exactAttributes(purpose, []); exactAttributes(payload, ["format"]); if (purpose.children.length || payload.children.length || payload.attributes.format !== "application/json") throw new Error("PACKAGE_ENVELOPE_INVALID"); if (["1", "2", "3"].includes(attributes.schema_version)) throw new Error(`WORKFLOW_PACKAGE_SCHEMA_MIGRATION_REQUIRED: schema_version ${attributes.schema_version} -> 4`); const value = validateWorkflowPackage(JSON.parse(payload.text)); if (attributes.schema_version !== String(PACKAGE_SCHEMA_VERSION) || attributes.key !== value.key || attributes.version !== value.version || purpose.text !== value.purpose) throw new Error("PACKAGE_ENVELOPE_MISMATCH"); return value; }
...
function applyWorkflows(db, proposal, projectId, value) {
  ...
  for (const workflow of value.workflows) {
    ...
    db.prepare(`INSERT INTO workflows(id,name,project_id,package_key,package_version,default_quality,default_level,status,discovery_json) VALUES(?,?,?,?,?,?,?,?,?)
      ON CONFLICT(id) DO UPDATE SET name=excluded.name,package_key=excluded.package_key,package_version=excluded.package_version,default_quality=excluded.default_quality,default_level=excluded.default_level,status=excluded.status,discovery_json=excluded.discovery_json`)
      .run(workflowId, workflow.name, projectId, value.key, value.version, workflow.default_quality, workflow.default_level, workflow.status, stableJson(workflow.discovery));
