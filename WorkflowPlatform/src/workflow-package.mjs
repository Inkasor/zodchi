...
export function validateWorkflowPackage(value) {
  if (value?.schema_version !== PACKAGE_SCHEMA_VERSION) {
    if (value?.schema_version === 1 || value?.schema_version === 2) throw new Error(`WORKFLOW_PACKAGE_SCHEMA_MIGRATION_REQUIRED: schema_version ${value.schema_version} -> 3`);
...
export function parseWorkflowPackage(source) { const root = parseLimitedXml(source); if (root.name !== "workflow_package") throw new Error("PACKAGE_ROOT_INVALID"); const attributes = exactAttributes(root, ["key", "version", "schema_version"]), [purpose, payload] = exactChildren(root, ["purpose", "payload"]); exactAttributes(purpose, []); exactAttributes(payload, ["format"]); if (purpose.children.length || payload.children.length || payload.attributes.format !== "application/json") throw new Error("PACKAGE_ENVELOPE_INVALID"); if (attributes.schema_version === "1" || attributes.schema_version === "2") throw new Error(`WORKFLOW_PACKAGE_SCHEMA_MIGRATION_REQUIRED: schema_version ${attributes.schema_version} -> 3`); const value = validateWorkflowPackage(JSON.parse(payload.text)); if (attributes.schema_version !== String(PACKAGE_SCHEMA_VERSION) || attributes.key !== value.key || attributes.version !== value.version || purpose.text !== value.purpose) throw new Error("PACKAGE_ENVELOPE_MISMATCH"); return value; }
