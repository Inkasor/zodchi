...
export function validateClassification(v) { assertFields(v, ["kind", "domain", "discipline", "risk", "level", "quality"], "classification"); if (!CLASSIFICATION_KINDS.includes(v.kind)) throw new Error("classification: invalid kind"); if (!LEVELS.includes(v.level)) throw new Error("classification: invalid level"); if (!QUALITY.includes(v.quality)) throw new Error("classification: invalid quality"); return v; }
