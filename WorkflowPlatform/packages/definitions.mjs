...
function indiePackage(projectKey) {
  ...
  const roles = [
    role("classifier", "Select only a registered Indie Studio route.", ["conversation", "clarification", "research", "game_design", "narrative", "implementation", "fix", "documentation", "art_direction", "audio", "prototype", "testing", "release", "producer"], ["none"], { schema: "classification.v1", corrections: 0 }),
  ...
  const workflows = [
    ...
    flow("prototype", "Prototype", [step("plan", 1, "planner", ["document"]), step("prototype", 2, "game_programmer", ["prototype"]), step("test", 3, "game_tester", ["test_report"])], [question("assumption", "Which single assumption does the prototype test?")], { quality: "prototype", level: "L1" }),
    flow("playtest", "Playtest", [step("build", 1, "release_operator", ["release_package"], codeChecks), step("playtest", 2, "playtester", ["test_report"]), step("owner_gates", 3, null, ["decision"], [], { irreversible: true })], [], { quality: "production", level: "L3" }),
    flow("release", "Release", [step("freeze", 1, "producer_assistant", ["decision"]), step("build", 2, "release_operator", ["release_package"], codeChecks), step("review", 3, "reviewer", ["test_report"], codeChecks), step("publication_approval", 4, null, ["decision"], [], { irreversible: true })], [question("release_authority", "Has the owner granted separate publication authority?")], { quality: "production", level: "L3" }),
...
function sharedMapPackage() {
  ...
  const checkKeys = checks.map(item => item.key), roles = [role("shared_engine_architect", "Protect the presentation-neutral consumer contract.", ["planning", "map_design"], ["document"], { skills: ["game-production:shared-map-engine"] }), role("shared_engine_programmer", "Implement an isolated presentation-neutral engine package.", ["implementation", "fix"], ["code"], { tools: ["apply_patch"], skills: ["game-production:shared-map-engine"], checks: checkKeys }), role("shared_engine_tester", "Run deterministic engine and consumer compatibility checks.", ["testing", "verification"], ["test_report"], { checks: checkKeys }), role("shared_engine_reviewer", "Review engine boundaries read-only.", ["review", "verification"], ["test_report", "code", "document"], { checks: checkKeys }), role("documentator", "Record the accepted contract version without product decisions.", ["documentation"], ["document"], { tools: ["apply_patch"] })];
  const steps = [step("contract", 1, "shared_engine_architect", ["document"]), step("implement", 2, "shared_engine_programmer", ["code"], checkKeys), step("test", 3, "shared_engine_tester", ["test_report"], checkKeys), step("review", 4, "shared_engine_reviewer", ["test_report"], checkKeys), step("document", 5, "documentator", ["document"])];
  const workflows = [
    workflow("shared-map-engine.change", "Shared map engine change", steps, [question("consumer_contract", "Which exact Project M and Project R consumer contracts are in scope?")]),
    workflow("shared-map-engine.verification", "Registered verification run", [step("checks", 1, "shared_engine_tester", ["test_report"], checkKeys), step("review", 2, "shared_engine_reviewer", ["test_report"], checkKeys)], [question("verification_scope", "Which registered checks should run, and over which paths?")], { level: "L1" }),
    workflow("shared-map-engine.documentation", "Registered documentation update", [step("document", 1, "documentator", ["document"]), step("review", 2, "shared_engine_reviewer", ["document"])], [question("document_outcome", "Which accepted contract version should be recorded?")])
  ];
  return finalize({ key: "shared-map-engine.core", version: "2.3.0", purpose: "Presentation-neutral shared map engine workflow with separate M/R compatibility and owner acceptance gates.", roles, workflows, routes: [route("map_design", "shared-map-engine.change"), route("implementation", "shared-map-engine.change"), route("fix", "shared-map-engine.change"), route("testing", "shared-map-engine.verification"), route("verification", "shared-map-engine.verification"), route("documentation", "shared-map-engine.documentation")], checks, operationalLevels: [{ level: "mvp", budgets: { calls: 10, duration_ms: 3600000 }, required_check_keys: checkKeys, correction_limit: 1, escalation: { consumer_compatibility_required: true } }], documents: [document("repo_rules", "AGENTS.md", "authority", "shared-map-engine", roles.map(item => binding(item.key))), document("package_contract", "package.json", "reference", "shared-map-engine", roles.map(item => binding(item.key)))], scenarios: [scenario("engine_change", { work_type: "implementation", artifact_type: "code" }, { isolated_workspace: true, consumers: ["project-m", "project-r"], human_acceptance_separate: true })] });
}
...
function lorePackage() {
  ...
  const roles = [role("lore_researcher", "Inventory exact existing facts and sources without changing canon.", ["research"], ["document"], { skills: ["game-production:shared-world-lore"], boundaries: { edits: false, invent_facts: false } }), role("lore_editor", "Propose a LORE-CHANGE without accepting it.", ["narrative", "decision"], ["decision", "document"], { skills: ["game-production:shared-world-lore"] }), role("continuity_reviewer", "Find terminology, chronology and M/R impact conflicts.", ["review", "verification", "documentation"], ["test_report", "document"], { skills: ["game-production:shared-world-lore"], checks: checkKeys }), role("lore_documentator", "Apply an owner-accepted LORE-CHANGE and update the index.", ["documentation", "decision"], ["document"], { tools: ["apply_patch"], skills: ["game-production:shared-world-lore"], checks: checkKeys })];
  // The change card is written before the owner decides, so a candidate that stays open or turns out
  // to be a conflict still leaves a durable record in docs/decisions without ever reaching canon.
  const steps = [step("research", 1, "lore_researcher", ["document"]), step("proposal", 2, "lore_editor", ["decision"]), step("continuity", 3, "continuity_reviewer", ["test_report"], checkKeys), step("change_card", 4, "lore_documentator", ["document"], checkKeys), step("owner_decision", 5, null, ["decision"], [], { irreversible: true }), step("canon", 6, "lore_documentator", ["document"], checkKeys), step("consumer_proposals", 7, "lore_editor", ["decision"])];
  const workflows = [
    workflow("shared-lore.change", "Shared lore change", steps, [question("shared_fact", "Which exact fact is proposed as shared canon?"), question("source_sha", "Which exact source SHA and section support the proposal?"), question("candidate_scope", "Is this candidate shared, M-only, R-only, a conflict, or still open?"), question("project_impact", "What exactly changes for Project M and for Project R, or none for either?")]),
    workflow("shared-lore.documentation", "Registered documentation update", [step("source", 1, "lore_researcher", ["document"]), step("document", 2, "lore_documentator", ["document"], checkKeys), step("review", 3, "continuity_reviewer", ["document"], checkKeys)], [question("document_outcome", "Which owner-accepted canon should be recorded?")])
  ];
  return finalize({ key: "shared-lore.canon", version: "2.3.0", purpose: "Shared Project Lore change protocol; Petr alone accepts canon and M/R synchronize separately.", roles, workflows, routes: [route("narrative", "shared-lore.change"), route("decision", "shared-lore.change"), route("documentation", "shared-lore.documentation")], checks, operationalLevels: [{ level: "prototype", budgets: { calls: 10, duration_ms: 2400000 }, required_check_keys: checkKeys, correction_limit: 0, escalation: { owner_canon_decision_required: true } }, { level: "mvp", budgets: { calls: 10, duration_ms: 2400000 }, required_check_keys: checkKeys, correction_limit: 1, escalation: { owner_canon_decision_required: true } }, { level: "production", budgets: { calls: 14, duration_ms: 4800000 }, required_check_keys: checkKeys, correction_limit: 1, escalation: { owner_canon_decision_required: true } }, { level: "security-audit", budgets: { calls: 10, duration_ms: 2400000 }, required_check_keys: checkKeys, correction_limit: 0, escalation: { owner_canon_decision_required: true } }], documents: [document("repo_rules", "AGENTS.md", "authority", "project-lore", roles.map(item => binding(item.key))), document("change_protocol", "docs/ChangeProtocol.md", "authority", "project-lore", roles.map(item => binding(item.key))), document("canon", "docs/Canon.md", "authority", "petr", roles.map(item => binding(item.key, item.key === "lore_documentator", "owner-accepted shared canon", 20))), document("lore_index", "data/lore-index.json", "authority", "project-lore", roles.map(item => binding(item.key, item.key === "lore_documentator", "accepted canon index", 10))), document("change_decisions", "docs/decisions", "authority", "project-lore", roles.map(item => binding(item.key, item.key === "lore_documentator" || item.key === "lore_editor", "LORE-CHANGE card for every candidate, accepted or not", 15)))], scenarios: [scenario("lore_change", { work_type: "narrative", candidate: "anonymized_fact" }, { continuity_review: true, owner_decision: "pending", consumer_updates: "separate" })] });
}

function oneCPackage() {
  const checks = withBroadSecretScan(completeSoftwareChecks([disabledCheck("bsl_language_server", "BSL Language Server diagnostics", "requires_local_bsl_binding", [checkBinding("mvp", "code")]), disabledCheck("one_c_local_build", "Stejmins 1C build", "requires_stejmins_build_binding", [checkBinding("production", "release_package")]), disabledCheck("one_c_runtime_1c", "Target 1C runtime", "requires_separate_runtime_authority", [checkBinding("production", "test_report")])], "bsl_language_server", "one_c"), "one_c"), sourceChecks = ["bsl_language_server", "one_c_secret_scan"];
  ...
  const roles = [role("one_c_analyst", "Define business outcome, current behavior, data, constraints and acceptance criteria.", ["research", "planning", "clarification", "documentation"], ["document", "decision"], { skills: [contextSkill], boundaries: { edits: false, business_acceptance: false } }), role("one_c_developer", "Implement only allowed BSL, form or metadata files while preserving encoding and invariants.", ["implementation", "fix"], ["code"], { tools: ["apply_patch"], skills: developerSkills, checks: sourceChecks }), role("one_c_tester", "Keep source, local build and server runtime evidence separate.", ["testing", "verification"], ["test_report"], { skills: [contextSkill, "epf-validate", "form-validate", "cfe-validate"], checks: sourceChecks }), role("one_c_reviewer", "Review production/security/high-risk or failed correction packages without business acceptance.", ["review", "verification"], ["test_report", "code", "document"], { skills: [contextSkill, workflowSkill], checks: sourceChecks }), role("one_c_documentator", "Update only registered plans and documentation after accepted decisions.", ["documentation", "decision"], ["document"], { tools: ["apply_patch"], skills: [contextSkill] })];
  const steps = [step("discovery", 1, "one_c_analyst", ["document"]), step("analysis", 2, "one_c_analyst", ["decision"]), step("owner_contract", 3, null, ["decision"], [], { irreversible: true }), step("implementation", 4, "one_c_developer", ["code"], sourceChecks), step("checks", 5, "one_c_tester", ["test_report"], sourceChecks), step("review", 6, "one_c_reviewer", ["test_report"], sourceChecks), step("document", 7, "one_c_documentator", ["document"]), step("user_acceptance", 8, null, ["decision"], [], { irreversible: true })], workflows = [
    workflow("one-c.change", "1C bounded change", steps, [question("business_outcome", "Which business outcome should change?"), question("current_behavior", "What behavior is observed now?"), question("data_contract", "Which data and invariants are affected?"), question("acceptance", "Which independent business, source, build, runtime, and user criteria apply?")]),
    workflow("one-c.verification", "Registered verification run", [step("checks", 1, "one_c_tester", ["test_report"], sourceChecks), step("review", 2, "one_c_reviewer", ["test_report"], sourceChecks)], [question("verification_scope", "Which registered checks should run, and over which paths?")], { level: "L1" }),
    workflow("one-c.documentation", "Registered documentation update", [step("source", 1, "one_c_analyst", ["document"]), step("document", 2, "one_c_documentator", ["document"]), step("review", 3, "one_c_reviewer", ["document"])], [question("document_outcome", "Which accepted decision should be recorded?")])
  ];
  return finalize({ key: "one-c.development", version: "2.3.0", purpose: "Portable 1C development workflow. BSL Language Server blocks only new critical correctness findings while maintainability findings remain visible; Stejmins build, target runtime, and user acceptance stay independent.", roles, workflows, routes: [route("implementation", "one-c.change"), route("fix", "one-c.change"), route("testing", "one-c.verification"), route("verification", "one-c.verification"), route("documentation", "one-c.documentation")], checks, operationalLevels: [{ level: "mvp", budgets: { calls: 8, duration_ms: 3600000 }, required_check_keys: sourceChecks, correction_limit: 1, escalation: { reviewer_on_high_risk_or_failed_cycle: true, business_acceptance_separate: true, runtime_acceptance_separate: true } }], documents: [document("repo_rules", "AGENTS.md", "authority", "advertising-project", roles.map(item => binding(item.key))), document("change_plan", "docs/PLAN.md", "plan", "project-lead", roles.map(item => binding(item.key, item.key === "one_c_documentator", "single active change package", 20))), document("production_plan", "docs/CURRENT_PRODUCTION.md", "plan", "project-lead", roles.map(item => binding(item.key))), document("ai_workflow", "docs/AI_WORKFLOW.md", "authority", "advertising-project", roles.map(item => binding(item.key))), document("testing_1c", "docs/TESTING_1C.md", "reference", "advertising-project", roles.map(item => binding(item.key)))], scenarios: [scenario("green", { classification: "implementation", project_check: "passed" }, { route: "one-c.change", state: "approval_required", response: "Technical checks passed; runtime and user acceptance remain separate." }), scenario("red", { classification: "implementation", project_check: "failed" }, { route: "one-c.change", state: "changes_requested", response: "The check failed; correction is limited to one cycle." }), scenario("timeout", { classification: "implementation", project_check: "timed_out" }, { route: "one-c.change", state: "blocked", response: "The check timed out; the result is not green." }), scenario("unavailable", { classification: "implementation", project_check: "unavailable" }, { route: "one-c.change", state: "blocked", response: "The BSL check is unavailable: requires_local_bsl_binding." })] });
}
...
function companyWebPackages() {
  ...
  const marketChecks = completeSoftwareChecks([
    ...
    commandCheck("marketplaces_health", "MarketplacesData collection health", "npm.cmd", ["run", "marketplace:health"], [checkBinding("mvp", "data_migration"), checkBinding("production", "deployment_evidence")], 600),
    // The evidence these three read lives in GitHub Actions and on the production VM, so the project
    // owns the server alias, paths and endpoint and Zodchi calls the registered script by name.
    commandCheck("marketplaces_ci", "MarketplacesData GitHub Actions", "npm.cmd", ["run", "verify:ci"], [checkBinding("production", "release_package")], 300),
    commandCheck("marketplaces_deployed_revision", "MarketplacesData deployed revision", "npm.cmd", ["run", "verify:deployed"], [checkBinding("production", "deployment_evidence")], 300),
    commandCheck("marketplaces_public_health", "MarketplacesData public health", "npm.cmd", ["run", "verify:health"], [checkBinding("production", "deployment_evidence")], 300)
  ...
  const operationsChecks = withBroadSecretScan([
  ...
  return [
    companyWebPackage({ key: "zodchi.product-development", version: "2.3.0", purpose: "Zodchi product development workflow with separate source, installed release and local data boundaries.", checks: zodchiChecks, codeChecks: ["zodchi_tests"], dataChecks: ["zodchi_tests"], releaseChecks: ["zodchi_tests", "zodchi_release_build"], documents: [
    ...
    companyWebPackage({ key: "company-web.marketplaces-data", version: "2.3.0", purpose: "Company workflow for MarketplacesData development, data collection, production incidents and controlled deployment.", checks: marketChecks, collection: true, codeChecks: ["marketplaces_tests", "marketplaces_build"], dataChecks: ["marketplaces_tests", "marketplaces_build", "marketplaces_health"], releaseChecks: marketChecks.map(item => item.key), documents: [
    ...
    companyWebPackage({ key: "company-operations.core", purpose: "Petr-owned administration, DevOps, access, project bootstrap, incident and security workflow for company infrastructure.", checks: operationsChecks, codeChecks: ["company_ops_secret_scan"], dataChecks: ["company_ops_secret_scan"], releaseChecks: operationsChecks.map(item => item.key), documents: [
