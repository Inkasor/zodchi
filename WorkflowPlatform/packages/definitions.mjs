...
const secretCheck = key => ({ key: `${key}_secret_scan`, name: "Проверка на секреты", runner: `${key}_secret_scan`, kind: "secret_scan", config: {}, timeout_seconds: 300, bindings: [checkBinding("security", "security_report")] });
const securityChecks = key => [
  secretCheck(key),
  commandCheck(`${key}_gitleaks`, "Gitleaks: история и рабочее дерево", "gitleaks.exe", ["detect", "--source", ".", "--redact", "--no-banner"], [checkBinding("security", "security_report")], 900),
  commandCheck(`${key}_osv`, "OSV: известные уязвимости зависимостей", "osv-scanner.exe", ["scan", "source", "-r", "."], [checkBinding("security", "security_report")], 1800)
];
...
function indiePackage(projectKey) {
  ...
  const workflows = [
    flow("indie_strategy", "Indie strategy", [step("producer_intake", 1, "producer_assistant", ["decision"]), step("research", 2, "researcher", ["document"], [], { required: false }), step("owner_decision", 3, null, ["decision"], [], { irreversible: true }), step("document", 4, "documentator", ["document"])], [question("target_outcome", "Какой ближайший результат должен оценить владелец?")]),
    flow("game_design", "Game design", [step("design", 1, "game_designer", ["document"]), step("document", 2, "documentator", ["document"]), step("owner_acceptance", 3, null, ["decision"], [], { irreversible: true })], [question("player_value", "Какую ценность должен получить игрок?")]),
    flow("narrative_design", "Narrative design", [step("narrative", 1, "narrative_designer", ["content_asset"]), step("owner_decision", 2, null, ["decision"], [], { irreversible: true }), step("document", 3, "documentator", ["document"])], [question("canon_choice", "Какое смысловое решение требуется принять владельцу?")]),
    flow("game_implementation", "Game implementation", [step("plan", 1, "planner", ["document"]), step("implement", 2, "game_programmer", ["code"], codeChecks), step("test", 3, "game_tester", ["test_report"], codeChecks), step("review", 4, "reviewer", ["test_report"], codeChecks), step("document", 5, "documentator", ["document"])], [question("allowed_paths", "Какие пути входят в ограниченный пакет?")]),
    flow("visual_asset", "Visual asset", [step("direction", 1, "visual_artist", ["visual_asset"]), step("visual_acceptance", 2, null, ["decision"], [], { irreversible: true }), step("technical", 3, "technical_artist", ["technical_art_spec"]), step("provenance", 4, "documentator", ["document"]), step("owner_acceptance", 5, null, ["decision"], [], { irreversible: true })], [question("visual_brief", "Какой визуальный brief уже принят владельцем?")]),
    flow("audio_asset", "Audio asset", [step("brief", 1, "sound_designer", ["audio_asset"]), step("technical", 2, "game_tester", ["test_report"]), step("integration", 3, "sound_designer", ["audio_asset"]), step("owner_acceptance", 4, null, ["decision"], [], { irreversible: true })], [question("audio_brief", "Какой audio brief уже принят владельцем?")]),
    flow("prototype", "Prototype", [step("plan", 1, "planner", ["document"]), step("prototype", 2, "game_programmer", ["prototype"]), step("test", 3, "game_tester", ["test_report"])], [question("assumption", "Какое одно предположение проверяет прототип?")]),
    ...
    flow("release", "Release", [step("freeze", 1, "producer_assistant", ["decision"]), step("build", 2, "release_operator", ["release_package"], codeChecks), step("review", 3, "reviewer", ["test_report"], codeChecks), step("publication_approval", 4, null, ["decision"], [], { irreversible: true })], [question("release_authority", "Подтверждена ли владельцем отдельная authority на публикацию?")])
  ];
  const documents = isM ? [
...
function oneCPackage() {
  ...
  const steps = [step("discovery", 1, "one_c_analyst", ["document"]), step("analysis", 2, "one_c_analyst", ["decision"]), step("owner_contract", 3, null, ["decision"], [], { irreversible: true }), step("implementation", 4, "one_c_developer", ["code"], sourceChecks), step("checks", 5, "one_c_tester", ["test_report"], sourceChecks), step("review", 6, "one_c_reviewer", ["test_report"], sourceChecks, { required: false }), step("document", 7, "one_c_documentator", ["document"]), step("user_acceptance", 8, null, ["decision"], [], { irreversible: true })], workflows = [workflow("one-c.change", "1C bounded change", steps, [question("business_outcome", "Какой бизнес-результат должен измениться?"), question("current_behavior", "Какое фактическое поведение наблюдается сейчас?"), question("data_contract", "Какие данные и инварианты затрагиваются?"), question("acceptance", "Какие отдельные business, source, build, runtime и user критерии применимы?")])];
  return finalize({ key: "one-c.development", version: "2.2.0", purpose: "Переносимый рабочий поток разработки 1С. BSL Language Server блокирует только новые критические ошибки корректности, а замечания о поддерживаемости остаются в отчёте; сборка Stejmins, целевая рабочая среда и пользовательская приёмка являются отдельными контурами.", roles, workflows, routes: [route("implementation", "one-c.change"), route("fix", "one-c.change"), route("testing", "one-c.change")], checks, operationalLevels: [{ level: "mvp", budgets: { calls: 8, duration_ms: 3600000 }, required_check_keys: sourceChecks, correction_limit: 1, escalation: { reviewer_on_high_risk_or_failed_cycle: true, business_acceptance_separate: true, runtime_acceptance_separate: true } }], documents: [document("repo_rules", "AGENTS.md", "authority", "advertising-project", roles.map(item => binding(item.key))), document("change_plan", "docs/PLAN.md", "plan", "project-lead", roles.map(item => binding(item.key, item.key === "one_c_documentator", "single active change package", 20))), document("production_plan", "docs/CURRENT_PRODUCTION.md", "plan", "project-lead", roles.map(item => binding(item.key))), document("ai_workflow", "docs/AI_WORKFLOW.md", "authority", "advertising-project", roles.map(item => binding(item.key))), document("testing_1c", "docs/TESTING_1C.md", "reference", "advertising-project", roles.map(item => binding(item.key)))], scenarios: [scenario("green", { classification: "implementation", project_check: "passed" }, { route: "one-c.change", state: "approval_required", response: "Технические проверки пройдены; runtime и пользовательская приёмка остаются отдельными." }), scenario("red", { classification: "implementation", project_check: "failed" }, { route: "one-c.change", state: "changes_requested", response: "Проверка не пройдена; исправление ограничено одним циклом." }), scenario("timeout", { classification: "implementation", project_check: "timed_out" }, { route: "one-c.change", state: "blocked", response: "Проверка превысила время; результат не считается зелёным." }), scenario("unavailable", { classification: "implementation", project_check: "unavailable" }, { route: "one-c.change", state: "blocked", response: "BSL-проверка недоступна: requires_local_bsl_binding." })] });
}
...
function companyWebPackage(spec) {
  ...
  return finalize({
    ...
    documents,
    scenarios: [
      scenario("natural_conversation", { message: "Поговори со мной о текущем состоянии", classifier: "model" }, { work_type: "conversation", productive_roles: [] }),
      scenario("bounded_research", { message: "Посмотри, почему текущий процесс неудобен", classifier: "model" }, { work_type: "research", roles: ["researcher"], excludes: ["planner", "web_developer", "reviewer"] }),
      scenario("implementation", { message: "Нужно исправить ограниченную проблему", classifier: "model" }, { work_type: "implementation", route: `${prefix}.change`, required_gates: codeChecks }),
      scenario("deployment_approval", { message: "Проверь готовность и при необходимости уточни публикацию", classifier: "model" }, { work_type: "deployment", route: `${prefix}.release`, keyword_trigger: false, explicit_approval_step: "deployment_approval" }),
      scenario("incident_separation", { message: "В рабочей системе ошибка, разберись", classifier: "model" }, { work_type: "incident", route: `${prefix}.incident`, deployment: "separate" }),
      ...(spec.content ? [scenario("content_candidate", { message: "Подготовь материал и покажи на проверку", classifier: "model" }, { work_type: "content", route: `${prefix}.content`, owner_acceptance: "pending" })] : [])
    ]
  });
}
