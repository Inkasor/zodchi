...
export async function loadPackageDefinitions(file = repositoryPackageDefinitionsFile) {
  ...
  if (!statuses || typeof statuses !== "object" || Array.isArray(statuses)) throw new Error(`PACKAGE_DEFINITIONS_INVALID: ${definitionsFile} statuses must be an object`);
  ...
  for (const key of Object.keys(statuses)) if (!packageKeys.has(key) || !["support-grade", "preview", "local"].includes(statuses[key])) throw new Error(`PACKAGE_STATUS_INVALID: ${key}=${statuses[key]}`);
