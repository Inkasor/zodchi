...
export default function definePackages(b) {
  ...
  const software = composedPackage(
    coreLifecycle({
      key: "software.web-application", version: "1.4.0", purpose: "Portable Web application workflow for bounded source and data changes, evidence-grounded API-to-UI review, incidents, access and approved release.", rolePreset: "full",
  ...
  const oneC = composedPackage(
    coreLifecycle({
      key: "one-c.development", version: "1.4.0", purpose: "Support-grade 1C source diagnosis, change, integration, module build, functional verification and release with external evidence boundaries.", rolePreset: "reviewed",
  ...
  const unity = composedPackage(
    coreLifecycle({
      key: "game.unity", version: "0.5.0", purpose: "Executable preview for bounded Unity design research, C# change, build and technical QA with separate visual, gameplay and owner acceptance.", rolePreset: "reviewed",
  ...
  const gameWeb = composedPackage(
    coreLifecycle({
      key: "game.web", version: "0.5.0", purpose: "Executable preview for browser-game design, implementation and deterministic browser proof with separate technical and owner product acceptance.", rolePreset: "full",
  ...
  const dataAnalytics = composedPackage(
    coreLifecycle({
      key: "data.analytics", version: "0.5.0", purpose: "Executable preview for read-only data discovery, deterministic invariants and approval-bound migration preparation without persisting source rows or prompts.", rolePreset: "reviewed",
  ...
  const infra = composedPackage(
    coreLifecycle({
      key: "infra.operations", version: "0.5.0", purpose: "Executable preview for read-only operations, incident diagnosis and approval-bound access, restore and delivery changes with redacted receipts.", rolePreset: "reviewed",
  ...
  const marketing = composedPackage(
    coreLifecycle({
      key: "marketing.content-operations", version: "0.5.0", purpose: "Executable preview for grounded research, edited content and planned-to-measured marketing activity without multiplying permanent roles or documents.", rolePreset: "full",
