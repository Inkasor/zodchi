...
for (const { item, workflowId, checks, directProfiles } of prepared) {
  ...
  results.push({ project_id: item.project_id, run_id: outcome.run_id, route: outcome.route, execution_status: outcome.execution?.status ?? null, execution_error: outcome.execution?.error ?? null, gate_status: outcome.execution?.gate?.status ?? null, final_state: statistics.final_state, calls: statistics.calls.length, tokens: statistics.tokens, worktree_unchanged: true, checks: checks ? { check_id: checks.check_id, kind: checks.kind, baseline_id: checks.baseline.baseline_id, tool_version: checks.baseline.tool_version, accepted_revision: checks.baseline.accepted_revision, confirmed_by: item.checks.one_c_bsl.confirmed_by } : null, owner_acceptance: "pending_separate" });
}
