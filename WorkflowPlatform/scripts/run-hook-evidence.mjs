...
let db = openDb(dbFile); const projectRoot = path.resolve(config.root_path); db.prepare("INSERT INTO projects(id,name,root_path,created_at) VALUES(?,?,?,?)").run(config.project_id, config.name, projectRoot, now()); registerImplicitResources(db, { projectId: config.project_id, rootPath: projectRoot }); db.close();
...
for (const scenario of scenarios) {
  const execution = spawnSync(process.execPath, [path.join(repositoryRoot, "hooks", "user-prompt-submit.mjs"), "--delivery-mode=advisory"], { cwd: path.resolve(config.root_path), encoding: "utf8", windowsHide: true, env: baseEnv, input: JSON.stringify(hookInput(scenario, path.resolve(config.root_path))) });
  ...
  if (!hook.additionalContext) throw new Error(`HOOK_WITHOUT_TOP_LEVEL_CONTEXT ${scenario.key}: ${JSON.stringify(hook)}`);
