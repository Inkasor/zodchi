...
const scenarios = [{ key: "conversation", event: "hook-conversation", message: "SCENARIO:conversation Привет!" }, { key: "research", event: "hook-research", message: "SCENARIO:research Кратко проверь зарегистрированный технический контекст Project R." }], results = [];
for (const scenario of scenarios) {
  const execution = spawnSync(process.execPath, [path.join(repositoryRoot, "hooks", "codex-user-prompt-submit.mjs")], { cwd: path.resolve(config.root_path), encoding: "utf8", windowsHide: true, env: baseEnv, input: JSON.stringify({ event_id: scenario.event, prompt: scenario.message, cwd: path.resolve(config.root_path) }) });
  if (execution.status !== 0) throw new Error(`HOOK_FAILED ${scenario.key}: ${execution.stderr || execution.stdout}`);
  const hook = JSON.parse(execution.stdout.trim()); db = openDb(dbFile); const runId = db.prepare("SELECT run_id FROM inbox_events WHERE project_id=? AND source='codex-hook' AND event_key=?").get(config.project_id, scenario.event)?.run_id; db.close();
  ...
  fs.writeFileSync(path.join(outputRoot, `${scenario.key}.statistics.json`), JSON.stringify(record, null, 2)); results.push({ scenario: scenario.key, run_id: runId, final_state: statistics.final_state, calls: statistics.calls.map(call => call.role), response_in_same_chat: Boolean(hook.hookSpecificOutput?.additionalContext) });
}
