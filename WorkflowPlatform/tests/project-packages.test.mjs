...
test("the canonical Web package is SDK-composed and requires anchored API-to-UI transitions", () => {
  ...
  assert.deepEqual(packageValue.roles.map(item => item.key).sort(), ["access_administrator", "adversarial_reviewer", "classifier", "conversation_responder", "coordinator", "editor", "evidence_reviewer", "judge", "release_operator", "researcher", "reviewer", "strategy_reviewer", "worker"]);
...
test("every package gives conversation its own read-only responder profile", () => {
  for (const packageValue of PACKAGE_DEFINITIONS) {
    const responder = packageValue.roles.find(item => item.key === "conversation_responder");
    assert.ok(responder, `${packageValue.key}: responder role`);
    assert.equal(responder.contract.result_schema_key, "conversation.v1");
    assert.equal(responder.contract.boundaries.context_input, true);
    assert.equal(responder.contract.boundaries.writes, false);
    assert.equal(responder.contract.boundaries.project_write, false);
    assert.equal(responder.contract.boundaries.external_mutation, false);
    assert.equal(responder.contract.boundaries.local_endpoint, false);
    assert.deepEqual(responder.contract.allowed_skills, []);
    assert.deepEqual(responder.contract.allowed_mcp_servers, []);
    assert.deepEqual(responder.contract.native_instruction_files, []);
    const profile = packageValue.profiles.find(item => item.role_key === "conversation_responder");
    assert.ok(profile, `${packageValue.key}: responder profile`);
    assert.equal(profile.key, `${packageValue.key}.conversation_responder.mvp`);
    assert.deepEqual(profile.capabilities, ["context_input"]);
  }
});
...
test("the Web-game preview traces design to browser proof without folding product acceptance into code review", () => {
  ...
  assert.deepEqual(packageValue.roles.map(item => item.key).sort(), ["adversarial_reviewer", "classifier", "conversation_responder", "coordinator", "editor", "evidence_reviewer", "judge", "researcher", "reviewer", "strategy_reviewer", "worker"]);
  assert.equal(packageValue.documents.length, 0);
...
test("the infrastructure preview has a two-step read-only route and approval-bound risky routes", () => {
  ...
  assert.deepEqual(packageValue.roles.map(item => item.key).sort(), ["access_administrator", "adversarial_reviewer", "classifier", "conversation_responder", "coordinator", "evidence_reviewer", "judge", "release_operator", "researcher", "reviewer", "strategy_reviewer", "worker"]);
  assert.equal(packageValue.documents.length, 0);
