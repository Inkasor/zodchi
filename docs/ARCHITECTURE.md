<document id="zodchi_architecture" status="accepted" authority="zodchi" version="0.6.14" language="en">
  ...
  <contracts status="accepted">
    ...
    <rule id="current_task_state">The current task context is a computed projection over canonical workflow tables, never a second state store. It preserves the verbatim owner objective, names strategic and tactical plans, decisions, questions, evidence and checks, and derives blockers and next actions without granting models authority over them.</rule>
    <rule id="validated_state_patch">A role migrated to StatePatch proposes only fields its contract names and binds the proposal to the exact projection hash it received. WorkflowPlatform validates and applies the patch atomically; stale patches and writes to derived fields fail closed.</rule>
    <rule id="addressed_history">Events and prior interactions remain append-only audit history and are retrieved by exact id when needed; the default role context is current state rather than a trailing event or message window.</rule>
    ...
    <rule id="session_isolation">The conditional client router keys activation by client and session id, refuses project rebinding, and emits no output before explicit activation. Claude Code receives prepared results through non-blocking hook context. Codex may omit that context, so its managed skill reads the prepared result through an immutable relay before any project work. The relay binds the exact client, session, generation and workflow run; a newer prompt invalidates the preceding result, and a late older generation cannot replace it. Activation checks the exact current `CODEX_SESSION_ID`; neither path substitutes a recent or project-wide session.</rule>
    ...
    <rule id="executor_capability_routing">Portable role contracts declare executor capabilities, while AgentGateway derives provider-specific technical, declarative, unavailable, and unknown guarantees from local profiles. Process execution, localhost access and external mutation are separate authority. Readiness and execution fail closed unless required capabilities are technically available and forbidden capabilities are technically absent.</rule>
    <rule id="model_input_boundary">Portable role contracts allowlist skills, MCP servers and native instruction files. A local external-tool registry describes endpoints, read-only modes, arbitrary execution, nested models, self-liftable boundaries, provider aliases and pinned versions. The gateway intersects the local profile with the role contract before launch and records a hashed inventory of every admitted or suppressed input in the receipt.</rule>
    <rule id="platform_owned_external_checks">HTTP, stdio, browser and SQLite verification is invoked by WorkflowPlatform outside the model. Missing endpoints, mismatched pinned versions and unverifiable output are unavailable, never passed.</rule>
    <rule id="browser_worker_boundary">Browser-facing packages may bind a dedicated worker that can change code and inspect the result through an explicitly allowed browser plugin in one bounded call. Model observation is implementation feedback, not deterministic browser evidence, product acceptance, or completion authority.</rule>
