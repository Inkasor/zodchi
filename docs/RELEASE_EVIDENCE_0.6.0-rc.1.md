...
|---|---|---|---|---|
| Web/TypeScript Dashboard | `run_1787986359294_8ea35ca7` | `completed`, gate `passed` | 3 calls; 192 input, 48 cached, 96 output, 12 reasoning | Clean revision `48739e23edf13001a44b7b896d5ef754917f6a54`: lint, secret scan and the complete 301-test project suite passed through the registered Web package. The worktree remained unchanged; merge, publication, deployment and owner acceptance remain separate. |
...
- The retained Dashboard evidence directory contains six files / 1,356,541 bytes with normalized inventory SHA-256 `c22ab184d24b671436265fa68d1dc25d98de756279e5168612c13de27d976186`. Its statistics bind the result to clean revision `48739e23edf13001a44b7b896d5ef754917f6a54` and record `web_lint` in 5,896 ms, secret scan in 110 ms and `web_tests` in 122,738 ms.
- Before the canary, one independent full `npm test` attempt reached the suite's 45-second timeout in `production API enforces administrator and read-only user boundaries` (300 passed, one cancelled). The exact test passed on immediate isolated rerun in about 24 seconds, and the subsequent canary's complete 301-test invocation passed. The timeout is retained as a flaky-performance observation, not treated as a waived gate or a deterministic product failure.
