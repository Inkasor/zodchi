...
3. Перегенерировать пользовательский каталог через `generate-packages.mjs --definitions <data-root>/packages/definitions.mjs`. Новый формат workflow package имеет schema v3 и prompt builder 3.4.1; поле `history_budget_bytes` больше не переносится, лимит контекста классификатора берётся из его role contract, а `strategy_review.v1` несёт hash-bound `StatePatch`. Перегенерация и импорт обязательны для всех трёх пользовательских пакетов в рамках того же обслуживания: до импорта обновлённых пакетов проекты `shared-lore` и `shared-map-engine` целиком имеют readiness `unavailable` с `missing_role_contracts=["classifier","researcher"]` и не принимают работу.
