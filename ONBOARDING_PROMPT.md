<document id="zodchi_onboarding" status="accepted" authority="zodchi" version="0.6.2" language="en">
  ...
  <project_onboarding status="accepted">
    ...
    <step order="21" id="review_strategy_selection">Run strategy-list automatically and show the effective strategy for every active package and quality level. Explain the practical choice before asking: standard uses the ordinary bounded execution and correction allowance; Gauntlet repeatedly runs deterministic gates, routes a failed gate to correction, and checks again until green or a bounded honest blocker. Review admission is independent: when the quality contract requires review, standard uses the primary reviewer and Gauntlet may use primary, adversarial, and evidence reviewers; judge and strategy reviewer remain conditional. Prototype defaults to Gauntlet with three correction cycles but keeps reviewer_policy=none. Ask the owner to confirm or override the defaults, apply the answer with strategy-set, and explain that the same setup chat can later handle the plain-language request “show or change this project's review mode”. Use inherit to return to package policy. Do not equate quality mode with review strategy or Gauntlet with mandatory review.</step>
  </project_onboarding>
