<document id="zodchi_onboarding" status="accepted" authority="zodchi" version="0.3.0-beta.4" language="en">
  ...
  <chat_entry_hook status="accepted">
    ...
    <step order="2" id="plan_hook">Run cli.mjs hook-plan --project &lt;root&gt; --harness codex|claude-code. It fills the release template, reports every conflict it found and states the mode it will use. Never write the hook configuration by hand: the file is shared with whatever else the project has configured, and rewriting it removes hooks that belong to other tools.</step>
    <step order="3" id="write_config">Run cli.mjs hook-install with the same arguments. It replaces only the entry Zodchi owns and leaves every other key, event and entry in place, then records ownership in .codex/.zodchi-hook.json or .claude/.zodchi-hook.json. If the plan reports proxy mode, the file is generated by another tool: show the person the entry and the event and ask them to add it to the source that generates the file, and do not write the generated file. The hook command holds a machine-specific absolute path, so it belongs in the local settings file, never in the shared .claude/settings.json and never in the user-wide settings file, which would fire the hook in unrelated projects.</step>
    ...
    <rule id="exec_form_hook_command">Configure the Claude Code hook in exec form: command "node" plus the script path as a single args element. A command string without args is passed to a shell, and on Windows that shell removes the backslashes of the release path, so the hook fails with a module-not-found error.</rule>
    <rule id="ignore_local_hook_config">Add .claude/settings.local.json, .claude/worktrees/, .claude/.zodchi-hook.json and .codex/.zodchi-hook.json to the project .gitignore so no local path reaches the project history. The ownership records hold the installation root of this machine, and .codex/hooks.json holds it in the command itself, so a project that commits its Codex hook configuration commits one machine's paths to everyone.</rule>
