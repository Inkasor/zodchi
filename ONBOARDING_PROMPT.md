<document id="zodchi_onboarding" status="accepted" authority="zodchi" version="0.6.11" language="en">
  ...
  <project_onboarding status="accepted">
    ...
    <rule id="openrouter_profile">When OpenRouter is selected, confirm that OPENROUTER_API_KEY exists without reading or printing its value, ask the owner which OpenRouter model slug may be used, create a local profile under the named openrouter provider, and run one bounded structured-output smoke. Require a schema-valid response and record tokens, cache fields, cost when supplied, duration, model, and provider. Until that succeeds, report the profile as preview/unverified and do not assign it to a role.</rule>
  ...
  <chat_entry_skill status="accepted">
    ...
    <step order="3.1" id="cursor_preview_entry">Cursor support is preview until a real owner-machine smoke passes. Cursor discovers the installed Zodchi skill from the managed global skill roots. Select `/zodchi`, use Alt+Enter on Windows/Linux or Option+Enter on macOS to keep it as the chat's Custom Mode, and keep that mode attached for every Zodchi turn. Verify sessionStart, beforeSubmitPrompt, and sessionEnd hooks against one exact conversation id and generation id. A workspace with zero or several roots is unavailable rather than guessed.</step>
