<document id="zodchi_onboarding" status="accepted" authority="zodchi" version="0.6.12" language="en">
  ...
  <preflight status="accepted">
    ...
    <check id="cursor_host">When Cursor is selected, verify the installed Cursor version and that its Hooks and Agent Skills features are available. Cursor Desktop presence alone is not a successful Zodchi host smoke.</check>
  ...
  <project_onboarding status="accepted">
    ...
    <step order="11" id="local_assignments">Show the verified harness/provider/model inventory, ask which systems the owner permits Zodchi to use, and ask about cost, privacy, and role preferences that cannot be inferred. Propose the smallest role assignment that satisfies the selected workflows; do not enable every available model merely because it is installed. Match every profile to the role contract's writes boundary: a role with writes=false must use a read-only profile, and a role with writes=true must use a writable profile. The direct chat runtime always requires both a classifier and a read-only researcher assignment: research is a valid classifier outcome even when the selected package does not contain a researcher step. Documentators are read-only model roles: they return a documentator.v1 proposal, while WorkflowPlatform validates, applies, lints, and rolls back the registered document change. Create a local installation config from configs/installation.example.json only from confirmed assignments. Never include tokens, cookies, passwords, or authentication files.</step>
    ...
    <rule id="openrouter_profile">OpenRouter support is preview. When OpenRouter is selected, confirm that OPENROUTER_API_KEY exists without reading or printing its value, ask the owner which OpenRouter model slug may be used, create a local profile under the named openrouter provider, and run one bounded structured-output smoke. Require a schema-valid response and record tokens, cache fields, cost when supplied, duration, model, and provider. Until that succeeds on the current installation, report the profile as preview/unverified and do not assign it to a role. An installation without OpenRouter does not block release of the stable product.</rule>
  ...
  <chat_entry_skill status="accepted">
    ...
    <step order="3.1" id="cursor_preview_entry">Cursor support is preview and remains unavailable on an installation until a real smoke passes there. Cursor discovers the installed Zodchi skill from the managed global skill roots. Select `/zodchi`, use Alt+Enter on Windows/Linux or Option+Enter on macOS to keep it as the chat's Custom Mode, and keep that mode attached for every Zodchi turn. Verify sessionStart, beforeSubmitPrompt, and sessionEnd hooks against one exact conversation id and generation id. A workspace with zero or several roots is unavailable rather than guessed. An installation without Cursor does not block release of the stable product.</step>
    ...
    <rule id="activation_delivery">Activation and prepared Zodchi results appear as normal non-blocking host turns. Claude Code consumes the hook context directly. Codex may omit that context, so its managed skill reads the immutable result relay for the exact current `CODEX_SESSION_ID`. Cursor's Custom Mode reads the same relay for the exact conversation id supplied by sessionStart, while beforeSubmitPrompt binds the current generation id. In all hosts the skill emits only the prepared response before commentary, tools, or project work. A new prompt invalidates the preceding result, and an older concurrent turn cannot overwrite the current one. Installed hooks never render successful work as a blocked user message.</rule>
