...
export function buildRelease({ repositoryRoot, output, stageRoot = os.tmpdir(), replace = false }) {
  ...
  try {
    ...
    for (const script of ["canary-checks.mjs", "explicit-invoke.mjs", "generate-bsl-diagnostic-catalog.mjs", "generate-packages.mjs", "project-baseline.mjs", "run-e2e-evidence.mjs", "run-explicit-evidence.mjs", "run-hook-evidence.mjs", "run-owner-boundary-evidence.mjs", "run-package-boundary-evidence.mjs"]) copyFile(path.join(repository, "WorkflowPlatform"), `scripts/${script}`, path.join(stage, "WorkflowPlatform"));
