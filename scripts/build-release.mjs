...
export function buildRelease({ repositoryRoot, output, stageRoot = null, replace = false }) {
  ...
  try {
    ...
    for (const file of ["browser-proof.mjs", "browser-sentinel.mjs", "claude-skill-boundary-smoke.mjs", "codex-browser-smoke.mjs", "codex-readonly-smoke.mjs", "install.mjs", "installation-paths.mjs", "kimi-write-boundary-smoke.mjs", "local-endpoint-boundary-smoke.mjs", "mcp-browser-smoke.mjs", "profile-policy-audit.mjs", "profile-policy-prune.mjs", "skill-installation.mjs", "session-hook-installation.mjs", "install-or-update.ps1", "install-latest.ps1", "install-latest.mjs", "install-latest.sh", "release-smoke.mjs", "platform-acceptance.mjs"]) copyFile(repository, `tools/${file}`, stage, `tools/${file}`);
