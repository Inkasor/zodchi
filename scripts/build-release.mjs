...
export function buildRelease({ repositoryRoot, output, stageRoot = os.tmpdir(), replace = false }) {
  ...
  try {
    ...
    for (const file of ["install.mjs", "installation-paths.mjs", "skill-installation.mjs", "install-or-update.ps1", "install-latest.ps1", "install-latest.mjs", "install-latest.sh", "release-smoke.mjs", "platform-acceptance.mjs"]) copyFile(repository, `tools/${file}`, stage, `tools/${file}`);
