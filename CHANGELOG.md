...
- Browser-facing Web and Web-game packages use a dedicated `browser_worker`. The same bounded role may change code and inspect the result through an explicitly allowed Codex browser or Chrome plugin plus `node_repl`; 1C, Unity, data and infrastructure packages do not inherit this requirement from generic external-runtime support. Model observation remains separate from the registered deterministic browser gate and owner acceptance.
