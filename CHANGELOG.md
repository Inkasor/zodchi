...
- Post-operation verification preserves the run's already authorized change scope and validates operation payload semantics only after signature verification. Pre-action infrastructure review no longer receives an impossible source-evidence chain about an operation that has not happened yet, and the Windows Codex read-only smoke now retains its Gateway database, receipt row and raw policy rejection.
