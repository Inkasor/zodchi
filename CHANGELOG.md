...
- `software.web-application` now binds only its productive worker to the locally registered, version-pinned `playwright` MCP server; researcher and review remain unbound, while platform-owned checks stay independent. The analytics read-only probe reads the registered SQLite catalog, and scalar expectations require exactly one returned cell.
...
- Direct research now has three distinct outcomes: an evidence-backed `answered` result completes, a valid `insufficient` result is recorded and blocks without claiming success, and a provider or transport failure fails the run under its technical category.
- WorkflowPlatform now builds a bounded, relevance-ranked source packet for direct research using the registered roots and source scope. The researcher receives selected source excerpts and completeness metadata instead of relying on shell access; an answered result may cite only content actually supplied, while inventory-only or missing paths fail closed.
- Software research separates controlled documents from implementation evidence. Source selection excludes prose documents, prefers source paths related to explicit project identifiers, and requires at least one actually supplied source file before an `answered` result can complete; dry-run defers the content scan until execution.
- Prose research now learns cross-language technical terms from subject-bearing project lines and ranks rare, path-affine source matches ahead of generic symbols. Source evidence is required consistently for every catalog discipline that can make implementation claims, not only software and 1C.
- Cross-language terms contribute to ordinary relevance instead of overriding it, nearby prose lines may carry one bounded terminology bridge, and generated/build trees no longer compete with implementation sources. Within equally relevant matches, conventional implementation directories lead tests, migrations and scripts without excluding them.
- Prose source ranking now keeps a wider bounded corpus vocabulary, deduplicates case variants and treats only multi-term basename matches as path evidence. A real-repository regression set covers external operations, research source delivery and insufficient-research state handling instead of relying only on ideal fixtures.
- Production research and its real-corpus regression now share one exported ranking contract for expansion, candidate count and final selection, so tests cannot silently exercise different search limits from live intake.
- The shared research ranking contract retains forty corpus-owned identifier candidates before source ranking, keeping rare compound implementation names stable as ordinary documentation and readiness code grow without widening the file scan or returned source packet.
...
- The write boundary is now part of an explicit executor-capability contract covering embedded context, project reading and search, language services, process and long-lived-process access, project writes, network, browser automation, screen capture, MCP and skills. Gateway derives provider-specific status, enforcement and access for each profile; declarative or unknown guarantees do not satisfy a technically required capability.
- Browser-facing Web workers may change code and inspect the result in the same bounded cycle when their assigned profile provides an explicitly allowed Codex Browser or Chrome plugin plus `node_repl`. Browser automation and screen capture are optional enhancements, so their absence never blocks implementation, fixes, testing or verification; deterministic browser gates and owner acceptance remain separate authorities.
- External-runtime support no longer grants workers generic `exec_command`: project commands and browser proofs run as registered deterministic platform checks, while incident, backup/restore and activity-operation roles retain command execution where their contracts require it.
- Generated package profile `capabilities` now names required executor capabilities rather than the legacy allowed-skills list. Optional browser capabilities remain in role boundaries and do not become readiness prerequisites.
- Browser-capable worker profiles may explicitly name an installed Browser/Chrome plugin and `node_repl`, but configuration alone remains `unknown` and no ambient desktop plugin is inherited. The corrected Windows smoke imports the plugin's current `browser-client`, initializes its runtime inside the child CLI and records that no Browser/extension backend was connected in that run; this is a dynamic backend absence, not a missing Gateway allowlist or API. Screen capture is forbidden where a role contract explicitly disallows it; long-lived-process confinement remains unavailable until a provider can enforce that boundary technically.
- The browser capability smoke no longer requires the capability it is trying to prove. It serves an owner-unknown sentinel through a one-shot local HTTP endpoint, admits only the ordinary writable worker contour, and confirms browser automation only when the platform independently observes browser navigation, subresource loading and JavaScript execution in addition to the exact rendered values and allowlisted receipt components. A model-reported screenshot remains `unknown` until an external contour returns a verifiable capture artifact.
- Browser-smoke reports are versioned and state their evidence limit explicitly: the sentinel sequence raises the cost of accidental false positives but remains deliberately replayable, while overall browser status follows platform observation and never a screenshot self-report. Retained schema-v1 evidence remains valid only for the unavailable backend observed in that historical run.
- A provider-neutral browser contour can now name one locally registered Playwright-compatible MCP server through `browserMcpServer`. Codex and OpenCode carry only allowlisted native registrations; Claude receives only the selected definitions through an ephemeral `--mcp-config --strict-mcp-config`. Missing registrations fail before model work, Kimi and Cursor cannot claim this contour, and browser/capture capabilities remain unknown until their separate evidence requirements pass.
- The registered MCP contour has its own provider-neutral smoke. It verifies the server was actually carried, observes the shared browser sentinel, and records a retained PNG's size and SHA-256 when capture succeeds; the report states that a writable model can still deliberately replay or forge these signals, so project gates and owner acceptance remain independent.
- The MCP smoke recognizes direct structured results as well as current Claude CLI result envelopes and fenced JSON, so a technically observed sentinel does not become inconclusive merely because the provider wrapped the same bounded payload.
- Claude calls now pass registered role schemas through the CLI's native `--json-schema` value, and the MCP smoke may retain its screenshot at an explicit project-contained evidence path. This keeps the model result machine-readable and lets Playwright's allowed-root boundary produce a verifiable artifact without granting access to the system temporary directory.
- Persisted MCP evidence replaces machine-local project and temporary roots inside the provider's structured explanation with portable markers, while the retained artifact itself uses a project-relative path and digest.
- Claude receipts now describe their environment as `mcp-config-only` instead of claiming an ephemeral home: only the strict MCP configuration is isolated, while ambient Claude settings, hooks and skills remain outside that guarantee. Browser MCP registration mismatches use the profile/capability exit code, and screenshot evidence requires an IHDR viewport of at least 800×600 rather than accepting any file with a PNG signature.
- Screen-capture evidence now decodes the retained PNG and requires the run-specific random 8×8 sentinel marker, closing the gap where an unrelated viewport-sized image could qualify. The report identifies the MCP registration as isolated probe policy rather than implying installation in the owner's live policy; Claude's native `--json-schema` handling remains a product-wide provider contract, not a smoke-only option.
- A profile whose non-writing promise is only instructional does not silently qualify for a role that forbids project writes. A local profile may now register a capability-and-role-specific `acceptedDeclarativeBoundaries` exception with a mandatory owner-readable reason; it applies only to forbidden capabilities, never to required ones. Readiness and receipts expose the result as `accepted_declarative`, while every unlisted role continues to fail closed.
- Runtime readiness uses the Gateway-owned preflight at the point where each scope is known: classifier and researcher profiles are checked before intake, then only the selected workflow's roles at the selected operational level are checked before productive work. An incompatible release or access operator no longer disables unrelated routes. Installation and update diagnostics invoke the same Gateway command instead of maintaining a second policy-merge implementation; an absent `writes` declaration remains read-only.
...
- Post-operation verification preserves the run's already authorized change scope in both execution and its stored audit contract, and validates operation payload semantics only after signature verification. Release/access no longer receive a pre-action evidence chain about an operation that has not happened yet, while backup/restore retains its real observation-to-health flow.
...
- The validated owner installation completed its final schema-v3 regeneration under prompt builder 3.4.1 and now runs `zodchi.product-development` 2.9.2, `shared-lore.canon` 2.9.2 and `shared-map-engine.core` 2.5.2. Documentator and operator profiles are read-only; the two Kimi reviewer exceptions remain visible as eight `accepted_declarative` assignments, with no profile conflicts.
...
- Classifier semantic context now uses the active classifier role contract's `context_limit_bytes`; the unrelated workflow-level `history_budget_bytes` setting and database column are removed. Portable workflow packages move to schema v3, so schema-v1/v2 packages must be regenerated and version-bumped before import.
- Retained Windows probes for Kimi 0.40.1 show that workspace trust does not confine writes: with no trust record before or after either run, the model changed the requested project file through its native `Write` and `Edit` tools. The CLI's `--plan` mode cannot serve as an AgentGateway boundary because Kimi rejects combining it with the non-interactive `--prompt` contour. Reviewer write prohibitions therefore remain declarative and keep their explicit owner-accepted exceptions.
- AgentGateway now extracts argument-free tool-call metadata from structured provider event streams before transient output is bounded. WorkflowPlatform compares observed `apply_patch` and `exec_command` classes with the active role contract before accepting a result; a mismatch fails closed and remains visible in the receipt, while an unobservable transport stays `unavailable` rather than passing empty. Cursor preview uses its structured event stream so future live calls can be audited post-factum.
- Goals and stages now have an explicit owner control plane. Their verbatim titles are immutable, status transitions require named owner confirmation, and classified work binds only to one unambiguous active strategic path. Classifiers see the project catalog, while every downstream model role receives the current task binding; models cannot create or rewrite strategic truth themselves.
- WorkflowPlatform now computes a named current task-state projection from existing canonical tables, hashes it for optimistic concurrency, and exposes past interactions and events only by exact id. Role prompts receive a compact view derived from that projection. `strategy_reviewer` is the first migrated role: it may replace only the active strategy-recovery decision through `StatePatch`, and a stale hash or an attempt to patch derived blockers/next actions fails before application. Prompt builder 3.4.1 and regenerated package versions carry the updated result contract; `completionBlockers()` remains the sole completion authority.
...
- Codex Desktop's real `$zodchi` skill mention is recognized from the exact installed skill path; a similarly named or foreign Markdown link cannot activate Zodchi.
- Session-hook installation records the canonical skill path, uses Codex's three-second `SessionEnd` limit, and reports that changed unmanaged hooks require owner trust verification before they can run.
- Platform acceptance executes the installed Codex and Claude Code hook commands, proves that ordinary chats pass through, and proves activation and session cleanup against the workflow database.
- If a host expands the skill without running the hook, the skill fails closed instead of claiming that Zodchi mode is active.
...
- Planner locator fitting now removes duplicate global term and adapter catalogs while retaining the highest-ranked source path; an overflow records exact component byte metrics before failing closed.
- Semantic correction routing uses language-independent corpus rarity and identifier shape instead of Russian/English stoplists. Equivalent gaps in different human languages select the same source-bearing step when they share the same project symbol.
- Cross-layer evidence logic is isolated behind TypeScript, BSL and generic flow adapter contracts. Only adapters with deterministic transitions can turn shared endpoint symbols into observed edges; BSL remains explicitly unknown when that capability is unavailable.
- Package and runtime policy validation accept only one to three consilium members and require an active Judge contract for a multi-member Gauntlet.
