...
- Separate retained Windows Codex probes establish that the read-only contour exposes no native file reader and rejects local process execution by policy. The source-collection stage therefore has a verified capability reason instead of inferring file-read failure from a blocked shell command.
