...
- Browser-capable worker profiles may explicitly name an installed Browser/Chrome plugin and `node_repl`, but configuration alone remains `unknown` and no ambient desktop plugin is inherited. The retained Windows smoke carried both allowlisted components but found no `agent.browsers` API inside the child CLI's Node REPL, so that optional enhancement is currently unavailable in the local Gateway contour even though Browser, Chrome and Edge work in the interactive Codex app. Screen capture is forbidden where a role contract explicitly disallows it; long-lived-process confinement remains unavailable until a provider can enforce that boundary technically.
