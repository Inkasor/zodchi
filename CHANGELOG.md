...
- A profile whose non-writing promise is only instructional no longer qualifies for a role that forbids project writes. Existing Kimi reviewer assignments therefore need reassignment to a technically read-only or context-only profile; installation and route readiness list every affected assignment before model work starts.
