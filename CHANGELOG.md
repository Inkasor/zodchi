...
- Existing browser-worker assignments need a writable Codex profile whose local policy explicitly names an installed browser or Chrome plugin and `node_repl`, plus explicit technical capability entries backed by a successful smoke of that exact Gateway contour. Configuration alone remains `unknown`; profiles for roles without browser scope do not inherit ambient desktop plugins. The retained Windows smoke carried both allowlisted components but found no `agent.browsers` API inside the child CLI's Node REPL, so the current local Gateway profile is not yet eligible even though Browser, Chrome and Edge work in the interactive Codex app. Screen capture is forbidden where a role contract explicitly disallows it; long-lived-process confinement remains unavailable until a provider can enforce that boundary technically.
