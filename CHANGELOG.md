...
- Existing installations must assign read-only profiles to every role whose contract does not permit writes. A mismatched profile blocks only the direct runtime role or selected workflow that would use it; documentators are proposal-only, while release and access execution must be moved to the deterministic external control plane rather than granted model write access.
