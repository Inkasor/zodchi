...
- WorkflowPlatform now computes a named current task-state projection from existing canonical tables, hashes it for optimistic concurrency, and exposes past interactions and events only by exact id. Role prompts receive a compact view derived from that projection. `strategy_reviewer` is the first migrated role: it may replace only the active strategy-recovery decision through `StatePatch`, and a stale hash or an attempt to patch derived blockers/next actions fails before application. Prompt builder 3.4.0 and regenerated package versions carry the updated result contract; `completionBlockers()` remains the sole completion authority.
